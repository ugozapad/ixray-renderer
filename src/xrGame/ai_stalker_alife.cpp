...
void CAI_Stalker::remove_personal_only_ammo(const CInventoryItem* item)
{
	...
	for (const shared_str& ammo : weapon->m_ammoTypes)
	{
		bool found = false;
		...
		for (const PIItem item : inventory().m_all)
		{
			...
			const CWeapon* temp = item->cast_weapon();
			if (!temp)
				continue;

			if (std::find(temp->m_ammoTypes.begin(), temp->m_ammoTypes.end(), ammo) == temp->m_ammoTypes.end())
				continue;
		...
		for (const PIItem item : inventory().m_all)
		{
			if (xr_strcmp(ammo, item->object().cNameSect()))
			{
				continue;
			}
